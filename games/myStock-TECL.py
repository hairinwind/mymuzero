...
class MuZeroConfig:
    def __init__(self):
        ...
        ### Game
        self.observation_shape = (85, 123, 12)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
        ...
        self.batch_size = 64  # Number of parts of games to train on at each training step # by YAO, with batch_size increasing, it needs more memory
        ...
        if os.getenv('mymuzero_env') == 'dev': 
            self.training_steps = 30  # Total number of training steps (ie weights update according to a batch)
            self.batch_size = 5  # Number of parts of games to train on at each training step # by YAO, with batch_size increasing, it needs more memory

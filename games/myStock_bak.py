...
class MyCustomEnv(StocksEnv): 
    def _process_data(self):
        ...
        return prices, numpy.ndarray.flatten(signal_features)

    def _get_observation(self):
        print("_get_observation._current_tick: ", self._current_tick)
        print("_get_observation.window_size: ", self.window_size)
        return numpy.ndarray.flatten(self.signal_features[(self._current_tick-self.window_size):self._current_tick])

    def step(self, action):
        print("...MyCustomEnv...")
        observation, reward, done, info = StocksEnv.step(action)
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None):
        ...
        myStockEnv = MyCustomEnv(df=df, window_size=50, frame_bound=(50,200))
        env_maker = lambda: myStockEnv
